...
from countcv.core.fcrn_model import BaseModel, SAUnet
...
def load_mlflow_model(tracking_uri: str, run_id: str) -> tuple[BaseModel, SyncedTransform] | BaseModel:
...
) -> tuple[Path | None, BaseModel | None, SyncedTransform | None]:
	yolo_model_path = model_directory / "yolo" / "best.pt" if load_yolo else None

	if load_density:
		dens_model_path = model_directory / "cnn-base"
		with open(dens_model_path / "transform_config.yaml") as f:
			config_dict = yaml.safe_load(f)

			transform_config = SyncedTransformConfig.from_dict(config_dict)
			dens_transform = SyncedTransform(transform_config)
		try:
			dens_model = SAUnet(input_channels=3, tile_size=transform_config.tilesize)
			dens_model.load_state_dict(torch.load(dens_model_path / "model.pt", map_location=device))
		except RuntimeError:
			dens_model = SAUnet(input_channels=3, output_channels=3, tile_size=transform_config.tilesize)
			dens_model.load_state_dict(torch.load(dens_model_path / "model.pt", map_location=device))
		dens_model.eval()
	else:
		dens_model = None
