...
rm -rf data_subset
mkdir -p data_subset/data/synthetic_cells/raw
mkdir -p data_subset/data/uc_cells/labels
...
git lfs pull --include="data/synthetic_cells/raw/*" --exclude="*"
git lfs pull --include="data/uc_cells/raw/*" --exclude="*"
...
# Run Python sampler to select subset of files
echo "=== Running data sampler ==="
python3 ci/scripts/sample_data.py

# Copy metadata/config files
echo "=== Copying metadata files ==="
cp -v data/synthetic_cells/data.yaml data_subset/data/synthetic_cells/
cp -v data/uc_cells/data.yaml data_subset/data/uc_cells/
